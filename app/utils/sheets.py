...
def open_sheet():
    ...
    # Fallback: open by name (must exist in Drive)
    # We'll assume both sheets live in the same spreadsheet or separate ones with those names.
    # If they are separate spreadsheets, you can set SPREADSHEET_KEY and keep tab names consistent.
...
def get_worksheet(spreadsheet, name: str):
    try:
        ...
        # Try by title across all spreadsheets
        try:
            gc = get_client()
            ss = gc.open(name)
...
def append_message_log(spreadsheet, row: Dict[str, Any]):
    ...
    if not headers:
        ws.append_row([c.upper() for c in default_cols])
